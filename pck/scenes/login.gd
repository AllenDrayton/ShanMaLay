...
var remember_path = "user://remember_pressed.txt"
...
func _ready():
#	process_whole_url()
	$urlstore.text = "State = " + Config.STATE
	
	var file = File.new()
	if file.file_exists(remember_path):
		file.open(remember_path, File.READ)
...
func _store(data):
	var file = File.new()
	file.open(remember_path, File.WRITE)
